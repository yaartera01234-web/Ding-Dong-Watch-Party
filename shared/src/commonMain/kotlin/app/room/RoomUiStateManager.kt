...
class RoomUiStateManager(val viewmodel: RoomViewmodel) : AbstractManager(viewmodel) {
    ...
    /**
     * The message the composer is quoting. Set by swiping across a chat line; a non-null value
     * shows the reply bar above the input and prefixes the next sent message with the quoted
     * line, so the room (and any PC client) sees a plain-text reply.
     */
    val chatReplyTarget = MutableStateFlow<Message?>(null)
